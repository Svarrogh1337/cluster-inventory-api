...
test-conformance: ## Run the ClusterProfile API conformance suite (extra suite flags via CONFORMANCE_ARGS).
...
test-conformance-envtest: envtest ## Run the conformance suite against a temporary envtest control plane.
	...
	ENVTEST_PID=$$!; \
	trap 'kill $$ENVTEST_PID 2>/dev/null; wait $$ENVTEST_PID 2>/dev/null' EXIT; \
	...
	$(MAKE) test-conformance CONFORMANCE_ARGS="--kubeconfig $(ENVTEST_KUBECONFIG) $(CONFORMANCE_ARGS)"
...
test-conformance-kind: kind-conformance-up ## Run the conformance suite against the kind cluster, installing all requirements first.
	$(MAKE) test-conformance CONFORMANCE_ARGS="--context kind-$(KIND_CLUSTER_NAME) $(CONFORMANCE_ARGS)"
