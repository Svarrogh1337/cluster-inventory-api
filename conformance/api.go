...
// kep4322 is the specification the core conformance specs reference.
...
var _ = ginkgo.Describe("ClusterProfile API availability", func() {
	SpecifyWithSpecRef(fmt.Sprintf("The cluster must serve the %s resource in the %s API group version",
		cpv1alpha1.ClusterProfileSchemeGroupVersionResource.Resource, cpv1alpha1.GroupVersion),
		kep4322,
		ginkgo.Label(RequiredLabel), func(ctx context.Context) {
			resources, err := kubernetesClient.Discovery().ServerResourcesForGroupVersion(cpv1alpha1.GroupVersion.String())
			gomega.Expect(err).ToNot(gomega.HaveOccurred(), reportNonConformant(fmt.Sprintf(
				"the %s API group version is not served by the cluster", cpv1alpha1.GroupVersion)))

			gomega.Expect(slices.ContainsFunc(resources.APIResources, func(r metav1.APIResource) bool {
				return r.Name == cpv1alpha1.ClusterProfileSchemeGroupVersionResource.Resource
			})).To(gomega.BeTrue(), reportNonConformant(fmt.Sprintf(
				"the %s API group version does not serve the %s resource",
				cpv1alpha1.GroupVersion, cpv1alpha1.ClusterProfileSchemeGroupVersionResource.Resource)))

			_, err = clusterProfileClient.ApisV1alpha1().ClusterProfiles(namespace).List(ctx, metav1.ListOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred(), reportNonConformant(fmt.Sprintf(
