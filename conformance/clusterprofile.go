...
var _ = ginkgo.Describe("ClusterProfile", ginkgo.Label(clusterProfileAPI.Name), func() {
	// The suite has no registration input of its own: without a positive
	// --expected-clusters it can only verify that registered clusters are
	// represented at all.
	SpecifyWithSpecRef("The cluster manager must represent its registered member clusters as ClusterProfile "+
		"objects in the inventory namespace",
		...
		ginkgo.Label(RequiredLabel), func(ctx context.Context) {
			profiles := profilesUnderTest(ctx)
			gomega.Expect(profiles).ToNot(gomega.BeEmpty(), reportNonConformant(fmt.Sprintf(
			...
			if expectedClusters > 0 {
				gomega.Expect(profiles).To(gomega.HaveLen(expectedClusters), reportNonConformant(fmt.Sprintf(
					"%d clusters are registered (--expected-clusters) but the inventory namespace %q%s holds %d "+
						"ClusterProfile objects", expectedClusters, namespace, managerSuffix(), len(profiles))))
			}
		})
