...
func run(loadingRules *clientcmd.ClientConfigLoadingRules, kubeContext, namespace, clusterManager string,
	clusters int) error {
	if clusters < 0 {
		return fmt.Errorf("--clusters must not be negative; got %d", clusters)
	}
...
func seedClusterProfile(ctx context.Context, client cpclientset.Interface, namespace, clusterManager string,
	index int) error {
	name := fmt.Sprintf("%s-cluster-%d", objectNamePrefix(clusterManager), index)
	...
	if apierrors.IsAlreadyExists(err) {
		// Different cluster manager names can derive the same object name, so an
		// existing object is reused only when it belongs to the requested manager;
		// spec.clusterManager.name is immutable and cannot be taken over.
		created, err = client.ApisV1alpha2().ClusterProfiles(namespace).Get(ctx, name, metav1.GetOptions{})
		if err == nil && created.Spec.ClusterManager.Name != clusterManager {
			return fmt.Errorf("ClusterProfile %q already belongs to cluster manager %q; delete it or seed another "+
				"namespace for cluster manager %q", name, created.Spec.ClusterManager.Name, clusterManager)
		}
	}
	if err != nil {
...
// maxObjectNamePrefixLength keeps the derived object names and the
// inventory-member-id label values (name plus "-id") within the 63-character
// limit of label values.
const maxObjectNamePrefixLength = 40

// objectNamePrefix derives a DNS-safe, bounded prefix for the seeded object
// names from the cluster manager name, which is only constrained to be a label
// value: it may contain uppercase letters, underscores or dots and be up to 63
// characters long. spec.clusterManager.name itself keeps the raw value, as that
// is what the suite's --cluster-manager flag matches on.
func objectNamePrefix(clusterManager string) string {
	prefix := strings.Map(func(r rune) rune {
		...
		return '-'
	}, strings.ToLower(clusterManager))

	if len(prefix) > maxObjectNamePrefixLength {
		prefix = prefix[:maxObjectNamePrefixLength]
	}
